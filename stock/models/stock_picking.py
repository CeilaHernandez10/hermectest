...
class Picking(models.Model):
    ...
    state = fields.Selection([
        ('draft', 'Borrador'),
        ('waiting', 'Esperando Alguna Operacion '),
        ('confirmed', 'En Espera'),
        ('assigned', 'Listo'),
        ('done', 'Completado'),
        ('cancel', 'Cancelado'),
        ...
        help=" * Draft: not confirmed yet and will not be scheduled until confirmed.\n"
             ...
             " * Cancelled: has been cancelled, can't be confirmed anymore.")  #La linea 198 y 206 han sido modificadas
    ...
    priority = fields.Selection(
        ...
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},   #modificando el estado de 'done' a 'bodega'
    ...
    move_line_ids = fields.One2many('stock.move.line', 'picking_id', 'Operations')  #modificando el estado de 'done' a 'bodega'
    ...
    def _compute_show_lots_text(self):
        ...
        for picking in self:
            if not picking.move_line_ids:
                ...
                picking.show_lots_text = True      #modificando el estado de 'done' a 'bodega'
            else:
                picking.show_lots_text = False
    ...
    def _compute_state(self):
        ...
        - Bodega EMSA: if the picking is bodega. 
        - Cancelled: if the picking is cancelled
        ''' #modificando el estado de 'done' a 'bodega'
        ...
        for picking in self:
            ...
            for package_to_check in packages_to_check:
                if picking.state in ('done', 'cancel') or picking._check_move_lines_map_quant_package(package_to_check):
                    packages |= package_to_check  #modificando el estado de 'done' a 'bodega'
    ...
    def action_assign(self):
        ...
        moves = self.mapped('move_lines').filtered(lambda move: move.state not in ('draft', 'cancel', 'done')) #modificando el estado de 'done' a 'bodega'
        if not moves:
    ...
    def _create_backorder(self, backorder_moves=[]):
        ...
        for picking in self:
            moves_to_backorder = picking.move_lines.filtered(lambda x: x.state not in ('done', 'cancel')) #modificando el estado de 'done' a 'bodega'
    ...
    def _put_in_pack(self):
        package = False
        for pick in self.filtered(lambda p: p.state not in ('done', 'cancel')):  #modificando el estado de 'done' a 'bodega'
