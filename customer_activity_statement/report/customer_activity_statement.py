...
class CustomerActivityStatement(models.AbstractModel):
    ...
    def _get_account_invoice(self, ref):
        """
        [Codigo Anexado] Obtiene una factura usando como filtro su propiedad 
        number cuando es igual al param ref
        """
        return self.env['account.invoice'].search([("number", "=", ref)], limit=1)


    def _format_fiscal_number(self, number):
        """
        [Codigo Anexado] formatea el numero fiscal a 8 digitos de longitud
        """
    ...
    def _get_account_display_lines(self, company_id, partner_ids, date_start,
        ...
        for row in self.env.cr.dictfetchall():
            invoice = self._get_account_invoice(row["move_id"])
            row["type"] = "Invoice" if (invoice.type == "out_invoice") else "Credit Note"
            if type(invoice.fiscal_id) != bool and type(invoice.fiscal_reference) != bool:
                fiscal_number = invoice.fiscal_id + "-"
                fiscal_number = fiscal_number + self._format_fiscal_number(invoice.fiscal_reference)
                row["fiscal_number"] = fiscal_number
            else:
                row["fiscal_number"] = ""
